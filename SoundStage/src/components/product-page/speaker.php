...
<head>
    <meta charset="UTF-8">
    <title>Speakers | SoundStage</title>
...
<body>
    ...
    <div class="iem-main">
        <div class="container py-4">
            <h1 class="iem-title mb-3 mt-3 text-center"><i class="bi bi-speaker"></i> Speakers</h1>
            <div class="iem-desc-main mb-4">
                Fill your room with sound using our range of speakers.
            </div>
            <img src="/System/SoundStage/src/assets/images/speaker-banner.jpg" alt="Speaker Banner" class="iem-main-img mb-5">

            <?php foreach ($brands as $brand => $products): ?>
                <div class="iem-section-title"><?= htmlspecialchars($brand) ?></div>
                <div class="row g-4">
                    <?php foreach ($products as $spk): ?>
                        <div class="col-12 col-sm-6 col-md-4 col-lg-3">
                            <a href="product-page.php?id=<?= $spk['Product_ID'] ?>" class="iem-card-link" style="text-decoration:none; color:inherit;">
                                <div class="iem-card shadow-sm position-relative"
                                    data-id="<?= $spk['Product_ID'] ?>"
                                    data-name="<?= htmlspecialchars($spk['ProductName']) ?>"
                                    data-price="<?= $spk['Price'] ?>"
                                    data-image="/System/SoundStage/src/assets/speaker/<?= htmlspecialchars($spk['Image_URL']) ?>">
                                    <span class="iem-brand badge bg-primary position-absolute top-0 start-0 m-2"><?= htmlspecialchars($spk['Brand']) ?></span>
                                    <img src="<?= htmlspecialchars($spk['Image_URL']) ?>" alt="<?= htmlspecialchars($spk['ProductName']) ?>" class="iem-img card-img-top">
                                    <div class="iem-card-body p-3">
                                        <h5 class="iem-name mb-1"><?= htmlspecialchars($spk['ProductName']) ?></h5>
                                        <div class="iem-price mb-2">₱<?= number_format($spk['Price'], 0) ?></div>
