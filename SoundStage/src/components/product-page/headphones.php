...
// Fetch Headphones from product_tbl
$stmt = $pdo->prepare("SELECT * FROM product_tbl WHERE Category_ID = ? ORDER BY Brand, ProductName");
$headphonesCategoryId = 3; // Category_ID for Headphones
$stmt->execute([$headphonesCategoryId]);
$headphones = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Group by brand
$brands = [];
foreach ($headphones as $hp) {
    $brands[$hp['Brand']][] = $hp;
}
?>
<!DOCTYPE html>
...
<head>
    <meta charset="UTF-8">
    <title>Headphones | SoundStage</title>
...
<body>
    ...
    <div class="iem-main">
        <div class="container py-4">
            <h1 class="iem-title mb-3 mt-3 text-center"><i class="bi bi-headphones"></i> Headphones</h1>
            <div class="iem-desc-main mb-4">
                Experience immersive sound with our curated selection of headphones.
            </div>
            <img src="/System/SoundStage/src/assets/images/headphone-banner.avif" alt="Headphones Banner" class="iem-main-img mb-5">

            <?php foreach ($brands as $brand => $products): ?>
                <div class="iem-section-title"><?= htmlspecialchars($brand) ?></div>
                <div class="row g-4">
                    <?php foreach ($products as $hp): ?>
                        <div class="col-12 col-sm-6 col-md-4 col-lg-3">
                            <a href="product-page.php?id=<?= $hp['Product_ID'] ?>" class="iem-card-link" style="text-decoration:none; color:inherit;">
                                <div class="iem-card shadow-sm position-relative"
                                    data-id="<?= $hp['Product_ID'] ?>"
                                    data-name="<?= htmlspecialchars($hp['ProductName']) ?>"
                                    data-price="<?= $hp['Price'] ?>"
                                    data-image="/System/SoundStage/src/assets/headphones/<?= htmlspecialchars($hp['Image_URL']) ?>">
                                    <span class="iem-brand badge bg-primary position-absolute top-0 start-0 m-2"><?= htmlspecialchars($hp['Brand']) ?></span>
                                    <img src="<?= htmlspecialchars($hp['Image_URL']) ?>" alt="<?= htmlspecialchars($hp['ProductName']) ?>" class="iem-img card-img-top">
                                    <div class="iem-card-body p-3">
                                        <h5 class="iem-name mb-1"><?= htmlspecialchars($hp['ProductName']) ?></h5>
                                        <div class="iem-price mb-2">₱<?= number_format($hp['Price'], 0) ?></div>
