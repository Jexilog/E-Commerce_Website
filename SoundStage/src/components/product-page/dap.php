...
<head>
    <meta charset="UTF-8">
    <title>Digital Audio Players | SoundStage</title>
...
<body>
    ...
    <div class="iem-main">
        <div class="container py-4">
            <h1 class="iem-title mb-3 mt-3 text-center"><i class="bi bi-music-player"></i> Digital Audio Players (DAP)</h1>
            <div class="iem-desc-main mb-4">
                Take your music anywhere with our range of digital audio players.
            </div>
            <img src="/System/SoundStage/src/assets/images/dap-banner.webp" alt="DAP Banner" class="iem-main-img mb-5">

            <?php foreach ($brands as $brand => $products): ?>
                <div class="iem-section-title"><?= htmlspecialchars($brand) ?></div>
                <div class="row g-4">
                    <?php foreach ($products as $dap): ?>
                        <div class="col-12 col-sm-6 col-md-4 col-lg-3">
                            <a href="product-page.php?id=<?= $dap['Product_ID'] ?>" class="iem-card-link" style="text-decoration:none; color:inherit;">
                                <div class="iem-card shadow-sm position-relative"
                                    data-id="<?= $dap['Product_ID'] ?>"
                                    data-name="<?= htmlspecialchars($dap['ProductName']) ?>"
                                    data-price="<?= $dap['Price'] ?>"
                                    data-image="/System/SoundStage/src/assets/dap/<?= htmlspecialchars($dap['Image_URL']) ?>">
                                    <span class="iem-brand badge bg-primary position-absolute top-0 start-0 m-2"><?= htmlspecialchars($dap['Brand']) ?></span>
                                    <img src="<?= htmlspecialchars($dap['Image_URL']) ?>" alt="<?= htmlspecialchars($dap['ProductName']) ?>" class="iem-img card-img-top">
                                    <div class="iem-card-body p-3">
                                        <h5 class="iem-name mb-1"><?= htmlspecialchars($dap['ProductName']) ?></h5>
                                        <div class="iem-price mb-2">₱<?= number_format($dap['Price'], 0) ?></div>
