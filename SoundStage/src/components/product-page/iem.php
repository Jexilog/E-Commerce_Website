...
// Fetch IEMs from product_tbl (palitan ang Category_ID depende sa iyong DB)
$stmt = $pdo->prepare("SELECT * FROM product_tbl WHERE Category_ID = ? ORDER BY Brand, ProductName");
$iemCategoryId = 1; // Palitan ayon sa iyong Category_ID para sa IEMs
$stmt->execute([$iemCategoryId]);
$iems = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Group by brand
$brands = [];
foreach ($iems as $iem) {
    $brands[$iem['Brand']][] = $iem;
}
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
    ...
    <div class="iem-main">
        <div class="container py-4">
            <h1 class="iem-title mb-3 mt-3 text-center"><i class="bi bi-earbuds"></i> In-Ear Monitors (IEMs)</h1>
                            ...
                            <a href="product-page.php?id=<?= $iem['Product_ID'] ?>" class="iem-card-link" style="text-decoration:none; color:inherit;">
                                <div class="iem-card shadow-sm position-relative"
                                    ...
                                    <div class="iem-card-body p-3">
                                        <h5 class="iem-name mb-1"><?= htmlspecialchars($iem['ProductName']) ?></h5>
                                        <div class="iem-price mb-2">₱<?= number_format($iem['Price'], 0) ?></div>
